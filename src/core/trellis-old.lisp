...
(defun initialize-trellis (height width &key retain-complete-bp-table retain-complete-trellis hmm use-lm match-function insertion-penalty language-weight log-base)
  ...
  (let ((trellis (make-trellis :height height :width width :hmm hmm :use-lm use-lm :match-function match-function :insertion-penalty insertion-penalty :language-weight language-weight))
	;;(initial-back-pointer (make-back-pointer :word-id "<s>" :begin-time 0 :end-time 0 :cumulative-score 0.0 :bp nil))
    ...
    (setf (t-gaussian-match-cache trellis) (make-array height :initial-element nil :element-type '(or null single-float)))
...
(defun print-top-n-histories (trellis n &key in-progress)
  ...
  (let ((histories (get-top-n-histories trellis n :in-progress in-progress)))
    (loop for (history score count states) in histories
	 ...
	 (if in-progress
	     (format t "          ~:d. ~{~A ~} (~F) [count: ~:d] [states:~{ ~A~}]~%" i (mapcar 'cleanup-phone-string history) score count states)
...
(defun get-trellis-lm-score (state-id trellis)
  ...
  (let* ((hmm (t-hmm trellis))
	 ...
	 (new-word (aref (language-hmm-hmm-state-words hmm) state-id))
	 (lm-score (get-back-pointer-lm-score bp new-word)))
    ;;(format t "LM score: ~A~%" lm-score)
    (coerce lm-score 'single-float)))

(defun get-hmm-match-score-old (trellis emitting-state)
  ...
  (declare (optimize (speed 3)))
  (let* ((hmm (t-hmm trellis))
	 (hmm-state-distribution (svref (language-hmm-emission-distributions hmm) emitting-state))) ;; this should return null or a gaussian-mixture (or a gaussian...)
    (cond ((svref (t-gaussian-match-cache trellis) emitting-state)
	   (svref (t-gaussian-match-cache trellis) emitting-state))
	  (hmm-state-distribution
	   (setf (svref (t-gaussian-match-cache trellis) emitting-state) (funcall 'log-gaussian-mixture-probability (t-frame-features trellis) hmm-state-distribution))
	   (svref (t-gaussian-match-cache trellis) emitting-state))
	  (t (error "Impossible situation encountered.  State must be emitting or non-emitting.")))))
...
(defun search-illusory-trellis (input-sequence hmm &key
  ...
  (let* ((search-start-time (get-internal-real-time))
    ...
    (loop for i from 0 below width
	 ...
	 (when debug (format t "Working on column # ~5:D of ~5:D.  Active state count ~8:D.  Best score: ~f~%" (1+ i) width active-state-count best-score) (force-output))
	 ...
	 (cond ((and beam-threshold relative-threshold)
		(error "Can't perform beam threshold and relative threshold simultaneously!"))
	       (relative-threshold
		(prune-active-nodes (t-active-nodes trellis) (t-scores trellis) relative-threshold :verbose verbose))
	       (beam-threshold
...
(defun advance-by-one-state (state-id trellis &key (debug nil) (verbose nil))
  ...
  (declare (optimize (speed 3)))
  (let* ((hmm (t-hmm trellis))
	 (successors (elt (language-hmm-transitions hmm) state-id))
    ...
    (loop for successor of-type fixnum in successors
       for log-transition-probability of-type single-float in (aref (language-hmm-transition-probabilities hmm) state-id) do
	 ;;(format t "Transition score: ~F~%" log-transition-probability)
...
(defun push-to-state (begin-state end-state cumulative-score trellis previous-back-pointer &key (debug nil) (verbose nil));; recursion-depth)
  ...
  ;;(assert previous-back-pointer)
  (let* ((hmm (t-hmm trellis))
	 (emitting-state end-state) ;; seems like this could go either way...
	 (hmm-state-distribution (aref (the (simple-array (or null distribution)) (language-hmm-emission-distributions hmm)) emitting-state)) ;; this should return null or a gaussian-mixture (or a gaussian...)
	 (non-emitting-state-p (not hmm-state-distribution))
	 (lm-score nil)
	 (original-cumulative-score cumulative-score)
	 (previous-cumulative-score (if previous-back-pointer (back-pointer-cumulative-score previous-back-pointer) 0.0)))
    (declare ((or single-float null) lm-score)
	     (single-float original-cumulative-score))
    ;; if we're using the LM, and we're at the end of a word, and the word we're ending isn't a silence... then we need an LM score.
    (when (and (t-use-lm trellis)
	       (word-ending-transition-p hmm begin-state end-state)
	       (string-not-equal (aref (the (simple-array (or null string)) (language-hmm-hmm-state-words hmm)) begin-state) "<sil>"))
      (multiple-value-setq (cumulative-score lm-score) (update-cumulative-score-with-lm begin-state trellis cumulative-score)))
    ;;(format t "LM score: ~F~%" lm-score)
    ;; If we're at the end of a word, we need a new back pointer to pass along...
    (when (word-ending-transition-p hmm begin-state end-state)
      (let* ((word-ending (aref (the (simple-array (or null string)) (language-hmm-hmm-state-words hmm)) begin-state))
	     (back-pointer (make-back-pointer :word-id word-ending
					      ...
					      :lm-score lm-score
					      :acoustic-score (- original-cumulative-score (the single-float previous-cumulative-score)))))
	(setf previous-back-pointer back-pointer)))
	...
	(progn
	  (let* ((match-score (get-hmm-match-score trellis emitting-state)))
	    ...
	    (setf match-score (/ match-score (the single-float (log (the single-float (t-log-base trellis))))))
	    ;;(format t "Match score: ~F~%" match-score)
...
(defun prune-active-nodes (active-node-list this-column relative-threshold &key (verbose t))
